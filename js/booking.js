...
function renderParlourServicesList() {
    ...
    <div class="service-card">
      ...
      <div class="service-content">
        ...
        <div style="margin-bottom: 1.25rem;">
          <ul style="font-size: 0.82rem; color: var(--dark-muted); display: flex; flex-direction: column; gap: 4px;">
            ${srv.includes.map(inc => `<li style="display:flex; align-items:center; gap:6px;"><span style="color:var(--primary-gold)">✓</span> ${inc}</li>`).join('')}
          </ul>
        </div>

        <div class="service-footer">
          <span class="service-price">$${srv.price.toFixed(2)}</span>
          <button class="btn btn-primary btn-sm" onclick="openBookingWithService('${srv.id}')">
            Book Ritual
          </button>
        </div>
      </div>
  ...
  if (homeContainer) homeContainer.innerHTML = html.slice(0, 3);
  if (aboutServicesContainer) aboutServicesContainer.innerHTML = html;
}
